...
export default function ActivityTabDesktop({ currentUser, onOpenBook }) {
  ...
  if (loading) return <div style={{ textAlign: 'center', color: '#A39B90', fontSize: 16, padding: 60 }}>טוען…</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40, alignItems: 'start' }}>
          ...
          <Section title="מבוקשים" count={requestedBorrows.length}>
            {requestedBorrows.length === 0
              ...
              : requestedBorrows.map(b => (
                  <Row key={b.id} onClick={() => b.Books && onOpenBook(b.Books)}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontWeight: 600, fontSize: 15, color: '#2C2622', marginBottom: 1 }}>{b.Books?.title}</div>
                      <div style={{ fontSize: 13, color: '#7C756C' }}>מאת {b.Books?.author}</div>
                      <div style={{ fontSize: 13, color: '#A39B90', marginTop: 4 }}>מהמדף של <strong style={{ color: '#6B5440' }}>{b.Books?.Users?.name}</strong></div>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6, flex: 'none' }}>
                      ...
                      <button onClick={e => { e.stopPropagation(); removeBorrow(b) }} disabled={removingBorrow === b.id} style={btnGhost('#B24A3A')}>{removingBorrow === b.id ? '…' : 'הסר'}</button>
                    </div>
                  </Row>
                ))
            }
            ...
            {activeBorrows.length === 0
              ...
              : activeBorrows.map(b => (
                  <Row key={b.id} onClick={() => b.Books && onOpenBook(b.Books)}>
                    ...
                    <button onClick={e => { e.stopPropagation(); markReturned(b) }} disabled={returningBorrow === b.id} style={{ ...btnSolid, background: '#2E8B57', display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        <div>
          <Section title="רשימת קריאה" count={readingList.length}
            ...
            action={
              <button onClick={() => setShowAddRL(true)} style={{ width: 30, height: 30, borderRadius: 9, border: 'none', background: '#C05A3E', color: '#F7F5F1', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#F7F5F1" strokeWidth="2.8" strokeLinecap="round"><path d="M12 5v14M5 12h14" /></svg>
              </button>
            }
          >
                  ...
                  return (
                    <Row key={item.id}>
                      ...
                      <div onClick={() => book && onOpenBook(book)} style={{ flex: 1, minWidth: 0, cursor: book ? 'pointer' : 'default' }}>
                        <div style={{ fontWeight: 600, fontSize: 15, color: item.is_read ? '#A39B90' : '#2C2622', textDecoration: item.is_read ? 'line-through' : 'none', marginBottom: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{title}</div>
                        {author && <div style={{ fontSize: 13, color: '#7C756C' }}>מאת {author}</div>}
                        <div style={{ fontSize: 13, color: '#A39B90', marginTop: 2 }}>{source}</div>
                      </div>
...
const btnGhost = (color) => ({ border: '1.5px solid #E7E1D6', background: '#FFFFFF', borderRadius: 8, padding: '6px 12px', fontSize: 12.5, fontFamily: "'Source Sans 3',sans-serif", fontWeight: 600, color, cursor: 'pointer' })
const btnSolid = { border: 'none', background: '#C05A3E', borderRadius: 8, padding: '6px 12px', fontSize: 12.5, fontFamily: "'Source Sans 3',sans-serif", fontWeight: 600, color: '#F7F5F1', cursor: 'pointer' }

function Row({ children, onClick }) {
  return (
    <div onClick={onClick} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 0', borderBottom: '1px solid #ECE7DE', cursor: onClick ? 'pointer' : 'default' }}>
      {children}
    </div>
  )
}
  ...
  return (
    <div style={{ marginBottom: 34 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
        <span style={{ fontFamily: "'Lora',serif", fontWeight: 600, fontSize: 19, color: '#2C2622' }}>{title}</span>
